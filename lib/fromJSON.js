...
function fromJSON(value, type, path) {
  if (process.env.DISABLE_TCOMB !== 'true') {
  ...
  switch (kind) {
    ...
    case 'struct' :
      ...
      for (k in props) {
        if (props.hasOwnProperty(k)) {
          ret[k] = fromJSON(value[k], props[k], ( process.env.DISABLE_TCOMB !== 'true' ? path.concat(k + ': ' + getTypeName(props[k])) : null ));
        }
      }
      return new type(ret);
    ...
    case 'interface' :
      ...
      for (k in interProps) {
        if (interProps.hasOwnProperty(k)) {
          ret[k] = fromJSON(value[k], interProps[k], ( process.env.DISABLE_TCOMB !== 'true' ? path.concat(k + ': ' + getTypeName(interProps[k])) : null ));
        }
      }
      return ret;
    ...
    case 'list' :
      ...
      return value.map(function (element, i) {
        return fromJSON(element, elementType, ( process.env.DISABLE_TCOMB !== 'true' ? path.concat(i + ': ' + elementTypeName) : null ));
      });
    ...
    case 'tuple' :
      ...
      return value.map(function (element, i) {
        return fromJSON(element, types[i], ( process.env.DISABLE_TCOMB !== 'true' ? path.concat(i + ': ' + getTypeName(types[i])) : null ));
      });
    ...
    case 'dict' :
      ...
      for (k in value) {
        if (value.hasOwnProperty(k)) {
          ret[domain(k, ( process.env.DISABLE_TCOMB !== 'true' ? path.concat(domainName) : null ))] = fromJSON(value[k], codomain, ( process.env.DISABLE_TCOMB !== 'true' ? path.concat(k + ': ' + codomainName) : null ));
        }
      }
      return ret;
    ...
    case 'intersection' :
      var values = type.meta.types.map(function (type, i) {
        return fromJSON(value, type, ( process.env.DISABLE_TCOMB !== 'true' ? path.concat(i + ': ' + getTypeName(type)) : null ));
      });
      return type(
