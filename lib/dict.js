  ...
  function Dict(value, path) {
    ...
    for (var k in value) {
      if (value.hasOwnProperty(k)) {
        k = create(domain, k, ( process.env.DISABLE_TCOMB !== 'true' ? path.concat(domainNameCache) : null ));
        var actual = value[k];
        var instance = create(codomain, actual, ( process.env.DISABLE_TCOMB !== 'true' ? path.concat(k + ': ' + codomainNameCache) : null ));
